...
import com.tpshop.mallc.model.OrderButtonModel;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (order.getProducts()!=null && order.getProducts().size() > 1 ){
			...
			if (order.getProducts() != null && order.getProducts().size() == 1){
				...
				Glide.with(mContext).load(url).placeholder(R.drawable.icon_product_null).diskCacheStrategy(DiskCacheStrategy.SOURCE).into(holder.orderPicImgv);
