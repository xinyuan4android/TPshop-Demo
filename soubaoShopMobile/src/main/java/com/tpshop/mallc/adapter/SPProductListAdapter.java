 ...
 * @version V1.0
 */
package com.tpshop.mallc.adapter;
...
public class SPProductListAdapter extends BaseAdapter{
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        //设置数据到View
        //格式化价格
		if (!SPStringUtils.isEmpty(product1.getShopPrice())){
		...
		Glide.with(mContext).load(imgUrl1).placeholder(R.drawable.icon_product_null).diskCacheStrategy(DiskCacheStrategy.SOURCE).into(holder.picImgv1);
        ...
        if ((position * 2 + 1) < mProductLst.size()) {
            ...
            //设置数据到View
            //格式化价格
			if (!SPStringUtils.isEmpty(product2.getShopPrice())){
			...
			Glide.with(mContext).load(imgUrl2).placeholder(R.drawable.icon_product_null).diskCacheStrategy(DiskCacheStrategy.SOURCE).into(holder.picImgv2);
			...
			holder.priceTxtv2.setText("");
			holder.nameTxtv2.setText("");
			holder.view2.setOnClickListener(null);
			holder.picImgv2.setImageDrawable(null);
		}
