...
public class SPProductDetailCommentAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (!SPStringUtils.isEmpty(goodsComment.getAddTime())){
			String date = SPUtils.convertFullTimeFromPhpTime(Long.valueOf(goodsComment.getAddTime()));
			holder.dateTxtv.setText(date);
		}
		Glide.with(mContext).load(goodsComment.getHeadUrl()).placeholder(R.drawable.icon_product_null).diskCacheStrategy(DiskCacheStrategy.SOURCE).into(holder.headImgv);
	...
	private void buildProductGallery(LinearLayout gallery , SPGoodsComment goodsComment){
		...
		final List<String> images = goodsComment.getImages();
		if (SPCommonUtils.verifyArray(images)){
			for (int i = 0; i < images.size(); i++){
				String url = images.get(i);
				View view = LayoutInflater.from(mContext).inflate(R.layout.activity_index_gallery_item, gallery, false);
				ImageView img = (ImageView) view.findViewById(R.id.id_index_gallery_item_image);
				Glide.with(mContext).load(url).placeholder(R.drawable.icon_product_null).diskCacheStrategy(DiskCacheStrategy.SOURCE).into(img);
				img.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						if(clickListener!=null)clickListener.onClickListener(images);
					}
				});
