...
import com.tpshop.mallc.adapter.SPProductAttrListAdapter;
...
import com.tpshop.mallc.view.SPMobileScrollLayout;
...
public class SPProductDetailActivity extends SPBaseActivity implements SPPageView.PageListener {
	...
	public void  init(){
		...
		bottomDialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				View parent = (View) arrView.getParent();
		...
		arrView = LayoutInflater.from(this).inflate(R.layout.product_param_listview, null);
		attrLstv = (ListView) arrView.findViewById(R.id.product_attr_lstv);
		bottomDialog.setContentView(arrView);
	...
	@Click({R.id.like_lyaout , R.id.product_cart_rlayout , R.id.product_cart_btn , R.id.product_spec_aview,
			R.id.product_attr_aview, R.id.product_detail_aview, R.id.product_comment_aview , R.id.back_imgv})
	public void onButtonClick(View v) {
		switch (v.getId()){
			case R.id.like_lyaout:
				...
				String type =  "1" ;
				if (SPShopUtils.isGoodsCollected(mGoodsID)){//收藏 -> 取消收藏
	...
	public void refreshCollectButton(){
		...
		if (SPShopUtils.isGoodsCollected(mGoodsID)) {
			//收藏
	...
	public void refreshCollectDatta(){
		...
		if (SPMobileApplication.getInstance().isLogined) {
			SPPersonRequest.getGoodsCollectWithSuccess(new SPSuccessListener () {
				@Override
				public void onRespone(String msg, Object response) {
					SPMobileApplication.getInstance().goodsCollects = (List<SPCollect>) response;
					refreshCollectButton();
				}
			}, new SPFailuredListener() {
				@Override
	...
	private void buildProductGallery(List<String> gallerys){
		...
		for (int i = 0; i < gallerys.size(); i++){
			...
			imgUrls.add(url);
			Glide.with(this).load(url).placeholder(R.drawable.icon_product_null).diskCacheStrategy(DiskCacheStrategy.SOURCE).into(imageView);
			bindClickListener(imageView, new View.OnClickListener() {
				@Override
	...
	public void startupImagePreview(){
		...
		Intent previewIntent = new Intent(this, SPImagePreviewActivity_.class);
