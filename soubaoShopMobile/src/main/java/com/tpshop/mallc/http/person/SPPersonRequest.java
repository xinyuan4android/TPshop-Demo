...
public class SPPersonRequest {
        ...
        SPMobileHttptRequest.post(url, null, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                try {
                    ...
                    if (status > 0) {
                        if(SPValidate.velidateJSONArray(response.get(SPMobileConstants.Response.RESULT))){
                            consignees = SPJsonUtil.fromJsonArrayToList( response.getJSONArray(SPMobileConstants.Response.RESULT) , SPConsigneeAddress.class);
                        }
     ...
     */
    public static void getOrderDetailByOrderSN(String orderSN , final SPSuccessListener successListener, final SPFailuredListener failuredListener){
        assert(successListener!=null);
        assert(failuredListener!=null);
        ...
        SPMobileHttptRequest.post(url, params, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                try {
                    ...
                    String msg = (String) response.get(SPMobileConstants.Response.MSG);
                    if (status > 0) {
                        List<SPExchange> exchanges = new ArrayList<SPExchange>();
                        if(SPValidate.velidateJSONArray(response.get(SPMobileConstants.Response.RESULT))){
                            exchanges = SPJsonUtil.fromJsonArrayToList(response.getJSONArray(SPMobileConstants.Response.RESULT), SPExchange.class);
                        }
